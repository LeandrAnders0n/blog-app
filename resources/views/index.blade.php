        ...
        <div>
            <div class="image-row second-row">
                ...
                <a href="/blog/post/{{ $post->post_id }}" class="image-container">
                    <img class="image" src="{{ $post->image_path }}" alt="{{ $post->title }}">
                    <div class="button-text button" style="background-color: #3d72a6;">{{ $post->tag }}</div>
                </a>
                <a href="/blog/post/{{ $post->post_id }}" class="image-container">
                    <div>
                        <h4 class="ml-3">{{ $post->title }}</h4>
                        <p class="ml-3">{{ $post->introduction_content }}</p>
                        <div class="button-text read-more ml-3">READ MORE</div>
                    </div>
                </a>
                <br>
                @endforeach
